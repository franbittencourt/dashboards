...
2. **Capa (Home) com HTML:** o modelo já inclui a medida **`HTML Capa`** (tabela `Medidas`), que retorna o HTML da capa com os KPIs do portfólio embutidos. Para usá-la: Inserir → Mais Visuais → AppSource → instalar **"HTML Content"** (Daniel Marsh-Patrick) → arrastar a medida `HTML Capa` para o campo *Values* do visual. O plano de fundo atual (`vHomeHero`, retângulo azul-marinho) e o textbox `vHomeWelcomeText` podem ser removidos depois que o visual HTML estiver posicionado, ou mantidos como imagem de fundo (Formatar página → Fundo do Canvas → Imagem) com a foto real da empreiteira.
